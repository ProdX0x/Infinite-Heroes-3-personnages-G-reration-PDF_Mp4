            ...
            <div className="max-w-[900px] w-full bg-white p-4 md:p-5 rotate-1 border-[6px] border-black shadow-[12px_12px_0px_rgba(0,0,0,0.6)] text-center relative">
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-4 text-left">
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                        ...
                        {/* HERO UPLOAD */}
                        <div className={`p-2 border-4 border-dashed ${props.hero ? 'border-green-500 bg-green-50' : 'border-blue-300 bg-blue-50'} transition-colors relative group`}>
                            <div className="flex justify-between items-center mb-1">
                                <input 
                                    ...
                                    className={`font-comic text-lg uppercase font-bold text-blue-900 bg-transparent border-b border-blue-200 focus:border-blue-900 focus:outline-none w-2/3 placeholder-blue-300`}
                                />
                                {props.hero && <span className="text-green-600 font-bold font-comic text-xs animate-pulse">✓ READY</span>}
                            </div>
                            ...
                            {props.hero ? (
                                <div className="flex gap-3 items-center mt-1">
                                     ...
                                     <label className="cursor-pointer comic-btn bg-yellow-400 text-black text-xs px-2 py-1 hover:bg-yellow-300 transition-transform active:scale-95 uppercase">
                                         REPLACE
                                         <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onHeroUpload(e.target.files[0])} />
                                     </label>
                                </div>
                            ) : (
                                <label className="comic-btn bg-blue-500 text-white text-base px-2 py-2 block w-full hover:bg-blue-400 cursor-pointer text-center">
                                    UPLOAD HERO 
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onHeroUpload(e.target.files[0])} />
                                </label>
                            )}
                        ...
                        {/* CO-STAR UPLOAD */}
                        <div className={`p-2 border-4 border-dashed ${props.friend ? 'border-green-500 bg-green-50' : 'border-purple-300 bg-purple-50'} transition-colors`}>
                            <div className="flex justify-between items-center mb-1">
                                <input 
                                    type="text" 
                                    ...
                                    className={`font-comic text-lg uppercase font-bold text-purple-900 bg-transparent border-b border-purple-200 focus:border-purple-900 focus:outline-none w-2/3 placeholder-purple-300`}
                                />
                                {props.friend && <span className="text-green-600 font-bold font-comic text-xs animate-pulse">✓ READY</span>}
                            </div>
                        ...
                        {/* RIVAL UPLOAD */}
                        <div className={`p-2 border-4 border-dashed ${props.rival ? 'border-green-500 bg-green-50' : 'border-red-300 bg-red-50'} transition-colors`}>
                            <div className="flex justify-between items-center mb-1">
                                <input 
                                    type="text" 
                                    value={props.rivalName} 
                                    onChange={(e) => props.onRivalNameChange(e.target.value)}
                                    placeholder="VILLAIN NAME"
                                    className={`font-comic text-lg uppercase font-bold text-red-900 bg-transparent border-b border-red-200 focus:border-red-900 focus:outline-none w-2/3 placeholder-red-300`}
                                />
                                {props.rival && <span className="text-green-600 font-bold font-comic text-xs animate-pulse">✓ READY</span>}
                            </div>

                            {props.rival ? (
                                <div className="flex gap-3 items-center mt-1">
                                    <img src={`data:image/jpeg;base64,${props.rival.base64}`} alt="Rival Preview" className="w-16 h-16 object-cover border-2 border-black shadow-[2px_2px_0px_rgba(0,0,0,0.2)] bg-white rotate-[-1deg]" />
                                    <label className="cursor-pointer comic-btn bg-yellow-400 text-black text-xs px-2 py-1 hover:bg-yellow-300 transition-transform active:scale-95 uppercase">
                                        REPLACE
                                        <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onRivalUpload(e.target.files[0])} />
                                    </label>
                                </div>
                            ) : (
                                <label className="comic-btn bg-red-600 text-white text-base px-2 py-2 block w-full hover:bg-red-500 cursor-pointer text-center">
                                    UPLOAD VILLAIN 
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onRivalUpload(e.target.files[0])} />
                                </label>
                            )}
