        ...
        <div className={`book ${props.currentSheetIndex > 0 ? 'opened' : ''} transition-all duration-1000 ease-in-out`}
          ...
          {sheetsToRender.map((sheet, i) => (
              <div key={i} className={`paper ${i < props.currentSheetIndex ? 'flipped' : ''}`} style={{ zIndex: i < props.currentSheetIndex ? i : sheetsToRender.length - i }}
                  ...
                  <div className="front">
                      <Panel face={sheet.front} allFaces={props.comicFaces} isExporting={props.isExporting} onChoice={props.onChoice} onOpenBook={props.onOpenBook} onDownloadPDF={props.onDownloadPDF} onDownloadVideo={props.onDownloadVideo} onReset={props.onReset} />
                  </div>
                  <div className="back">
                      <Panel face={sheet.back} allFaces={props.comicFaces} isExporting={props.isExporting} onChoice={props.onChoice} onOpenBook={props.onOpenBook} onDownloadPDF={props.onDownloadPDF} onDownloadVideo={props.onDownloadVideo} onReset={props.onReset} />
                  </div>
              </div>
          ))}
      </div>
